...
def client_verifikasi_mahasiswa_via_api(nim, thread_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[{thread_name}] Error API untuk NIM {nim}: Status {response.status_code}")
    except requests.exceptions.Timeout:
        print(f"[{thread_name}] Permintaan ke API untuk NIM {nim} timeout.")
    except requests.exceptions.RequestException as e:
        print(f"[{thread_name}] Terjadi kesalahan saat melakukan permintaan: {e}")
    finally:
